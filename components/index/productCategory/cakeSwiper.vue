<script setup>
const { cakeProducts } = storeToRefs(useProductStore())

const indexCakeSwiper = ref(null);
const swiper = useSwiper(indexCakeSwiper, {
  // effect: 'creative',
  loop: true,
  slidesPerView: 1,
  spaceBetween: 25,
...
<template>
  <div class="product-swiper-banner text-primary mx-auto max-w-[390px]">
    <h3 class="text-2xl text-center font-bold mb-4">蛋糕</h3>

    <ClientOnly>
      <swiper-container ref="indexCakeSwiper" :init="false">
        <swiper-slide
          v-for="products in cakeProducts"
          :key="products.id"
          class="group"
        >
